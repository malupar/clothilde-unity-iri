...
class Cloth:
    def __init__(self,verts,faces,name="clothilde"):
        ...
        self.total_inxt = 0 #global number of iterations perfomed when calling simulate
        self.total_cols = 0 #global number of atempts perfomed when solving collisions
    ...
    def getPositionsUnity(self):
        phi_all = self.positions_all
        for _ in range(self.numIterSmooth):
    ...
    def setSimulatorParameters(self, dt = 1/240, tol = 0.008, 
                               rho = 0.1, delta = 0.1, alpha = 0.3,
                               kappa = 1*1e-4, str = 0.01*1e-4, shr = 20*1e-4,
                               mu_f = 0.2, mu_s = 0.5, thck = 1.2, numIterSmooth = 2):
        #solver parameters
        self.dt = dt #time step
        self.tol = tol #tolerance for constraints
        self.implicitEuler = False
        self.numIterSmooth = numIterSmooth
    ...
    def solveLCP(self):
        ...
        #iterative process
        error_l = -1; ii = 0
        while error_l < -self.tol and ii < 50:  
    ...
    def updateClosePairs(self,phi_mat):
        ...
        if (mov > self.mov_tol) or (self.total_inxt == 0): #only check when at least 1 node has moved more than mov_eps
    ...
    def simulate(self, u, control, l):
        ...
        n_inxt = 0; error_str = np.inf; error_shr = np.inf; 

        while (error_str > self.tol or error_shr > self.tol) and n_inxt < 15:

            #shearing
            phi, lambda_shr, error_shr = self.projectConstraints(self.shear,phi,u,control,
                                                                lambda_shr,self.shr,
                                                                update_chol,0.005,n_inxt%3)
        ...
        n_cols = 0; self.error_slf = -np.inf
        ...
        self.total_inxt += n_inxt
        self.total_cols += n_cols

        self.positions_all = self.Am@self.positions

        #warnings
        if self.total_inxt/(len(self.history_pos)-1) > 5.5:
           print("Warning: average of more than 5.5 iterations taken for inextensibility, for better performance reduce dt")